...
options_ = [('nxgenerated.100',10),('nxgenerated.500',5)]
options_ = [('nxgenerated.100',5),('nxgenerated.100',10)]
...
file_path = "./20181005114137_100y500"
...
generationSolution={}
paretoSolution={}
for element in options_:
    networkTopology = element[0]
    numApps = element[1]
    if not generationSolution.has_key(networkTopology):
        generationSolution[networkTopology]={}
    if not paretoSolution.has_key(networkTopology):    
...
figtitleStr = 'Objectives evolution'
...
for series in seriesList:
   ...
   #    ax.set_title('axes title')
        ...
        if isplot==0:
            plt.title(networkTopology + str(numApps*3*20)+' services', fontsize=16)
        if isplot==1:
            plt.title(networkTopology + str(numApps*3*20)+' services', fontsize=16)
        if isplot==4:
...
for series in seriesList:
    for element in options_:
        ...
        ax = fig.add_subplot(2,columfigures,isplot+1)
        fig.subplots_adjust(bottom=0.15)
        plt.grid()
